...
Ranks 1 to 23 as researched, with the §2 overlay applied in the last column.
...
*Plus one extra package: **Sky Wolf Toolkit** (aggregate of #6, 8 to 16), submitted after app #5 approves. Scope it at the origin root so `apps/index.html` is the launcher.*
...
| **2** | **Decide the origin: keep `sws-apps-9646d.web.app` or move to a branded domain.** | 1 to 2 h if moving | Baked into every TWA. Decide before any `bubblewrap init`. A branded domain directly mitigates the webview-of-a-website clause (§2d). |
...
| **10** | **Write `design/scenes.mjs` entry for seating-chart** and run `node design/shots.mjs seating-chart`. | **2 to 3 h** | Only 4 of 23 apps have scenes: `home-inventory`, `sub-plans`, `packing-list`, `qr-maker`. #2 has none. This is code, not drawing. |
| **11** | **Hand-differentiate the feature graphic for apps #1-#3** (see §7 concepts and §8 specs). | **3 to 4 h, or hire it** | The 23 generated graphics are one invariant layout, the exact repetitive signal a human reviewer sees. |
...
| **13** | **Cloud-app prerequisites**, account deletion flow (in-app + `/delete-account/` page + Cloud Function for recursive deletes), UGC reporting + terms interstitial, App Check. | **3 to 5 days of engineering** | Verified absent: no `deleteUser` import anywhere, no report/block UI (`grep` finds only `reportValidity` and `savedFlag()`), no `apps/delete-account/`, no terms page. Blocks #20-#23 only. Build it once at #20. |
...
- **Target audience:** **"Ages 18 and over" as the SOLE age group.** Do not select 13 to 15 or 16 to 17; Google states those brackets "may be considered to include children in some locales," which pulls Families policy in through the side door, and the four Firebase apps *cannot* satisfy Families requirements, because anonymous Firebase UIDs are exactly the persistent identifier it prohibits. Answer **NO** to Designed for Families and **NO** to Teacher Approved. Leave **Restrict Minor Access UNCHECKED**.
...
Terms 1 to 8 are the ones worth defending. **Do not add a keyword block to the description, Play penalises repetitive or unrelated keywords.**
...
1. **`design/scenes.mjs` entry for seating-chart**, ~60 lines describing 5 panels and the app state each needs. **2 to 3 h.** Blocks app #2 entirely. Then repeat per app as you go, ~1 to 2 h each.
...
| Phone screenshots | **Min 2, max 8**, JPEG or 24-bit PNG, **no alpha**, min dimension 320 px, max 3840 px, **aspect ratio capped at 2:1**. 1080×2400 is 2.22:1 and **will be rejected**. Use **1080×1920 (1.78:1)**, which is exactly what `design/shots.mjs` composes. Ship **4 to 8 showing populated state**, not 2 and not empty screens. |
