...
```js
{
  ...
  schoolYear: '2026 to 27',           // free text, prints
...
51. `optimalKeep` equals brute-force optimum on 4,000 randomised categories (2 to 8 items, all drop counts)
...
- Budget: ~1.1 to 1.3 s per student once the stack is marked. 24 students ≈ 30 s; a 32-student secondary section ≈ 40 s.

**Touch equivalent** (an iPad on a knee, a laptop on a knee at 4pm): the same list with a numeric keypad **docked at the bottom that never moves or resizes**, the studio's thumb-zone invariant, carrying 0 to 9, a decimal, a next arrow, and M / Ex / A keys.
...
4. **Does the standards-based grid ever get built?** It is the one deferral that genuinely excludes a segment: districts on standards-based report cards need a students × standards matrix, not a students × assignments one. It is ~20 to 25% more code, almost all view and print. v2 or never?
