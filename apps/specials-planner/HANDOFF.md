...
> **Jessie feedback round 2 2026-08-18 (laptop notes from work):** two items. (1) **Fit to screen actually works now** — the old mode was a trap: the CSS hid the weeknav (which held the only exit button), the `.fitbar` the CSS referenced was never added to the DOM, and `body.fitscreen header.app` matched nothing so the header stayed and shrank the grid. Now: the header genuinely hides, a fixed `#fitbar` (outside `#view`, wired once at boot) carries week arrows + a mode toggle + Exit, Escape exits (guarded so pickers/modals/editing keep the key), and `render()` strips the mode on any non-week view so Setup can never render headerless. The mode toggle answers her "computer screen vs laptop screen" ask by outcome, not device: `S.fitMode` `'all'` (whole week, no scroll) or `'width'` (fit the periods across, bigger text, scroll down); both measure the live viewport and re-fit on resize. In width mode `fitGrid` pins `.grid-scroll` to the visual (scaled) size or the page scrolls into blank space. (2) **Select many, restyle once** — `#p-multi` ("Select many"), Ctrl/Cmd-click, or drag-sweep selects boxes (period cells + notes; `selecting`/`selBoxes` module state, never in `S`); the palette strip becomes a toolbar applying to every held box: colour (one-off `S.fill`), **`S.dim`** (grey-out, `td.dimmed` = grayscale+fade), **`S.boxStyle`** (`{sz:'s'|'l', f:'serif'|'hand'|'mono'}` → `.sz-*`/`.f-*` classes on the box, print sizes have their own pt steps), Clear styling, Done. Both stores are keyed like `S.fill` (cycle-position / `n<iso>`), created in `ensureColourState()`, additive — old backups import untouched. In select mode boxes render `contenteditable="false"` + `tabindex="0"`; Space/Enter toggles in `onBoxKey`; week nav clears the selection. Suite is now **152 assertions**.

> **Jessie feedback round 2026-08-18:** three additive features from her notes. (1) **Editable column names** — `config.periodNames[]` (position i names period i+1, `''` = default "Period n"); the grid heading is a `contenteditable` span (`.pn-edit`, blur commits, Escape reverts, empty restores default) and the name flows through `periodName(p, lower?)` into aria labels, the copy bar, merge dialogs, the lunch dropdown and the CSV header. Setup also lists one text box per period (`periodNameInputs`), because the stacked phone layout hides the heading row. Custom names are user text — every HTML interpolation of `periodName()` must go through `esc()`. (2) **"Every period" scopes** — the cell picker's "Apply it to" now offers `one | day | all | dowall`; `day` writes one-offs for every period of that date, `dowall` with no dates writes a standing `fillDow` per period. (3) **Dated rules** — picker date inputs (native calendar); an *edited* range stores `S.fillRules` `{c,dow,p(0=all),from,to}` (cells) or `S.fillColRules` `{c,col,from,to}` (headings); untouched dates still land in the old standing stores so backups keep their old shape until the feature is used. Precedence in `fillFor`: one-off `fill` → dated rule naming the period → dated every-period rule → standing `fillDow` (newest rule wins inside a tier, so "red until break, green after" is two rules). Column headings resolve per week via `colFillId(key, w)` (rule active if it overlaps the week). `usedColourIds`/`colourUseCount`/`scrubColour` all know about rules. Suite is now **114 assertions**.

> **Design-review round 2026-08-09 (READ THIS FIRST — the storage format changed):** lessons are no longer keyed to the calendar date. They are keyed to **cycle position** — `cells["c<n>|<period>"]`, where `n` is the nth teaching day of the year — so cancelling a day now slides the lessons forward with the rotation letters instead of leaving every plan attached to the wrong letter for the rest of the year. No-class days keep their own boxes under `cells["d<ISO>|<period>"]`. `migrate()` rewrites old date-keyed backups on import, and `cellText()` still falls back to a date key so nothing a migration cannot place is ever lost. Same round: dark mode fixed (the grid was `#fff` under near-white ink), every grid size moved onto `--t-*` tokens so the comfort panel reaches it, `contenteditable="plaintext-only"` so typed line breaks survive, the copy bar became an in-flow toolbar with real scope and `SWS.undo`, a verified save with a `pagehide` flush and a loud failure warning, a stacked day-card layout under 52rem, a sticky Day column above it, one-week-on-one-page print, and a `.trust` stamp. Suite was **79 assertions** after that round.
...
| `test/specials-planner.test.mjs` | jsdom regression harness — **152 assertions, currently green** |
...
**Run `npm test` after ANY change. All 152 must be green before you commit.** The harness boots the real app in jsdom and covers: welcome flow, year generation, rotation math, special-day letter skipping, cell/notes persistence, the copy bar, backup round-trip with old/partial files, CSV shape and quoting, XSS inertness, setup reflow, and reset.
