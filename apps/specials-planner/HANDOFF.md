...
| `test/specials-planner.test.mjs` | jsdom regression harness — **114 assertions, currently green** |
...
**Run `npm test` after ANY change. All 114 must be green before you commit.** The harness boots the real app in jsdom and covers: welcome flow, year generation, rotation math, special-day letter skipping, cell/notes persistence, the copy bar, backup round-trip with old/partial files, CSV shape and quoting, XSS inertness, setup reflow, and reset.
