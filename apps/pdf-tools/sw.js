...
const VERSION = 'pdftools-v28';
const ASSETS = ["./","./app.js","./apple-touch-icon.png","./core.js","./icon.svg","./index.html","./privacy.html","./manifest.webmanifest","./vendor-pdf-lib.js", "./fonts/space-grotesk-latin.woff2", "./sws-prefs.js", "./sws-ui.js", "./sws-backup.js"];
self.addEventListener('install', (e) => {
  e.waitUntil(caches.open(VERSION).then((c) => c.addAll(ASSETS)).then(() => self.skipWaiting()));
});
self.addEventListener('activate', (e) => {
  e.waitUntil(caches.keys().then((keys) =>
    Promise.all(keys.filter((k) => k.startsWith('pdftools-') && k !== VERSION).map((k) => caches.delete(k)))).then(() => self.clients.claim()));
});
